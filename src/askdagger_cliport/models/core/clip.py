...
from askdagger_cliport.utils.simple_tokenizer import SimpleTokenizer as _Tokenizer


__all__ = ["available_models", "load", "tokenize"]  # noqa: F822
...
def build_model(state_dict: dict, device: Union[str, torch.device]):
    ...
    if type(device) is str and device != "cpu" or device.type != "cpu":
...
def load_clip(name: str, device: Union[str, torch.device], jit=False):
    ...
    if not jit:
        model = build_model(model.state_dict(), device).to(device)
        if str(device) == "cpu":
            model.float()
